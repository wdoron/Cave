...
namespace Dweiss
{
    namespace ReflectionWrapper
    {

        public static class ReflectionUtils
        {
            public static System.Type GetEnumType(string enumName, BindingFlags bf = PublicAndStaticBindingFlags)
            {

                var parts = enumName.Split('.');

                var ret = Type.GetType(enumName);
                if (ret == null)
                {
                    var prefix = string.Join(".", parts, 0, parts.Length - 1);
                    var justEnum = parts[parts.Length - 1];
                    var fullName = prefix + "+" + justEnum;
            ...
            public enum ReflectionSupportedTypes
            {
                String, Bool, Vector3, Int, Float
            }
          ...
          //  typeof(long),
            ...
            public static MemberClass[] GetPrimitiveMembers(Type type, BindingFlags bf = PrivateBindingFlags)
            {
            ...
            public static MemberClass GetPrimitiveMember(Type t, string name, BindingFlags bf = PrivateBindingFlags)
            {
            ...
            public static PropertyInfo[] GetPrimitiveProperties(Type type, BindingFlags bf = PrivateBindingFlags)
            {
            ...
            public static void SetField<T>(T script, string propName, object value, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                Debug.Log("Type " + t + " field " + propName);
                var prop = t.GetField(propName, bf);
                prop.SetValue(script, value);
            }

            public static void SetFieldOrProperty<T>(T script, string propName, object value, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                //Debug.Log("Type " + t + " field " + propName);
                var fld = t.GetField(propName, bf);
                if (fld != null)
                {
                    fld.SetValue(script, value);
                    return;
                }
            ...
            public static object GetField<T>(T script, string propName, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                var prop = t.GetField(propName, bf);
            ...
            public static Type GetFieldType<T>(T script, string propName, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                var prop = t.GetField(propName, bf);
                return prop.FieldType;
            }
            public static void SetProperty<T>(T script, string propName, object value, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                var prop = t.GetProperty(propName, bf);
            ...
            public static object GetProperty<T>(T script, string propName, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                var prop = t.GetProperty(propName, bf);
            ...
            public static Type GetPropertyType<T>(T script, string propName, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                var prop = t.GetProperty(propName, bf);
            ...
            public static Type[] GetMethodParamType<T>(T script, string propName, BindingFlags bf = PrivateBindingFlags)
            {
                var t = script.GetType();
                var prop = t.GetMethod(propName, bf);
                var prms = prop.GetParameters();
                return prms.Select(p => p.ParameterType).ToArray();
                //Debug.LogFormat("{0} - {1} = {2} ", t, propName, prop );
                //return prop.GetValue(script, null);
            }
            ...
            public static Type[] GetBaseTypes(this Type type)
            {
                ...
                {
                    var ienum = Enumerable.Repeat(type.BaseType, 1)
            ...
            public static object SetFunc<T>(T script, string memberName, object value, BindingFlags bf = PrivateBindingFlags)
            {
                // Debug.Log("Script " + script + " T " + typeof(T) + " mmbr " + memberName + " v " + value);
                var t = script.GetType();
                //Debug.Log("all " + t.GetBaseTypes().ToCommaString());
                var func = t.GetMethod(memberName, bf, null, new Type[] { }, null);//t.GetBaseTypes()
                //Debug.Log(" func " + (func==null?"NULL":func.ToString()) + " script " + script + " v " + value);
                //var func = t.GetMethod(memberName, bf);
                //var func = t.GetMethod(memberName, bf, null,  new Type[] { t.GetType()}, new ParameterModifier[0]);
            ...
            //////////////////////////////
            public static IEnumerable<MethodInfo> VisitMethods(Type t, BindingFlags bf = PrivateBindingFlags, bool addSpecailName = false)
            {
                ...
                foreach (var method in t.GetMethods(bf))
                {
                    ...
                    {
                        result.Add(method);
                        //valid += string.Format("{0}\nMOD {1}\n SPECAIL {2}\n MType {3}\n", method.Name, method.Module, method.IsSpecialName, method.MemberType);
                    }
                    else
                    {
                        // str += string.Format("{0}\nMOD {1}\n SPECAIL {2}\n MType {3}\n", method.Name, method.Module, method.IsSpecialName, method.MemberType );
            ...
            public static MethodInfo[] GetPrimitiveMethods(Type type, BindingFlags bf = PrivateBindingFlags)
            {
            ...
            /// <returns></returns>
            public static PropertyInfo[] GetProperties<T>(BindingFlags bf = PrivateBindingFlags)
            {
            ...
            public static MethodInfo[] GetFunctions<T>(BindingFlags bf = PrivateBindingFlags)
            {
                var type = typeof(T);
