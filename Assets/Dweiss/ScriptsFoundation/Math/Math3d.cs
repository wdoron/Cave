...
public static class Math3d {
	...
	public static Quaternion AverageQuaternion(this IList<Quaternion> list) {
		//var counter = 0;
		Quaternion ret = list[0], firstQ = list [0];
		var cumulative = new Vector4(firstQ.x, firstQ.y, firstQ.z, firstQ.w);
		for (int i=1; i < list.Count; ++i) {
			ret = AverageQuaternion (ref cumulative, list[i], firstQ, list.Count);
		}
		return ret;
    ...
    private static float LineLineDeltaIntersectionMin = 1e-9f, LineLineDeltaIntersectionMax = .0001f;
    ...
    public static bool LineLineIntersection(out Vector3 intersection, Vector3 linePoint1, Vector3 lineVec1, 
        ...
        , bool pointMustBeOnSector = false)
    {
        ...
        if (Mathf.Abs(planarFactor) < LineLineDeltaIntersectionMax && crossVec1and2.sqrMagnitude > LineLineDeltaIntersectionMin)
        {
            Vector3 crossVec3and2 = Vector3.Cross(lineVec3, lineVec2);
            float s = Vector3.Dot(crossVec3and2, crossVec1and2) / crossVec1and2.sqrMagnitude;
            intersection = linePoint1 + (lineVec1 * s);
            var pointInLine = true;
            if (pointMustBeOnSector)
            {
                var p1ToP3 = intersection - linePoint1;
                var dist = p1ToP3.magnitude / lineVec1.magnitude;
                pointInLine = Mathf.Abs((linePoint1 + dist * lineVec1 - intersection).magnitude) < LineLineDeltaIntersectionMax;

            }
            Debug.LogFormat("Intersects {6} +success - dot {0} cross {1}\n" +
                "v1 {2} v2 {3} v3 {4}\n crossVec1and2 {5}\n P {7} ", planarFactor, crossVec1and2.sqrMagnitude,
                lineVec1.ToMiliString(), lineVec2.ToMiliString(), lineVec3.ToMiliString(), crossVec1and2,
                pointInLine?"InSector" : "InfinitLines", intersection);
            return pointInLine;
        }
        else
        {
            Debug.LogFormat("Intersects fail - dot {0} cross {1}\n" +
               "v1 {2} v2 {3} v3 {4}\n crossVec1and2 {5}\n ", planarFactor, crossVec1and2.sqrMagnitude,
               lineVec1.ToMiliString(), lineVec2.ToMiliString(), lineVec3.ToMiliString(), crossVec1and2);
