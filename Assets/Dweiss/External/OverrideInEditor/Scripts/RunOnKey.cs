...
namespace Dweiss
{
    ...
    {
        public bool runOnlyInEditor = false;

        private bool showInfo = true;

        [Tooltip("Max time between each letter input before the input automatically fails")]
        private float maxTimeBetweenLetters = 3f;

        private bool fixedUpdateExecute = false;
        ...
        {
            for (int i=0; i < itemsToChangeOnUpdate.Length; ++i)
            {
                var item = itemsToChangeOnUpdate[i];
                try
                {
                    var b = item.Button;
                    if (item.IsValid() == false)
                    {
                        Debug.LogError( name + " Skipping invalid binding " + i + ". Possible gameobject changed or binding changed after setup. " + item);
                        continue;
                    }
                    ...
                    switch (b.bType)
                    {
                        case ItemsToChange.UiButtonType.Button:
                            item.go.GetComponent<UnityEngine.UI.Button>().onClick.AddListener(delegate { Execute(item); });//() => item.Override(true, "Canvas");
                            break;
                        case ItemsToChange.UiButtonType.NumberButtonInt:
                            {
                                var ifButtons = b.go.GetComponent<InputFieldWithButtons>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, (int)va); };

                                var v = item.GetValue();
                                ifButtons.ChangeValue((int)v);
                            }
                            break;
                        case ItemsToChange.UiButtonType.NumberButtonFloat:
                            {
                                var ifButtons = b.go.GetComponent<InputFieldWithButtons>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, va); };

                                var v = item.GetValue();
                                ifButtons.ChangeValue((float)v);
                            }
                            break;

                        case ItemsToChange.UiButtonType.NumberFuncInt:
                            {
                                var ifButtons = b.go.GetComponent<InputFieldWithButtons>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, (int)va); };
                            ...
                            break;
                        case ItemsToChange.UiButtonType.NumberFuncFloat:
                            {
                                var ifButtons = b.go.GetComponent<InputFieldWithButtons>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, (float)va); };
                            }
                            break;

                        case ItemsToChange.UiButtonType.BoolButton:
                            {
                                var ifButtons = b.go.GetComponent<BoolButton>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, va); };//() => item.Override(true, "Canvas");

                                var v = item.GetValue();
                                ifButtons.ChangeValue((bool)v);
                            }
                            break;
                        case ItemsToChange.UiButtonType.BoolFunc:
                            {
                                var ifButtons = b.go.GetComponent<BoolButton>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, va); };//() => item.Override(true, "Canvas");
                            ...
                            break;
                        case ItemsToChange.UiButtonType.EnumFunc:
                            {
                                var ifButtons = b.go.GetComponent<EnumButton>();
                                ifButtons.onValueChange += (va) => { ExecuteEnum(item, va); };//() => item.Override(true, "Canvas");
                            }
                            break;
                        case ItemsToChange.UiButtonType.EnumButton:
                            {
                                var ifButtons = b.go.GetComponent<EnumButton>();
                                ifButtons.onValueChange += (va) => { ExecuteEnum(item, va); };//() => item.Override(true, "Canvas");

                                var v = item.GetValue();
                                ifButtons.ChangeValue(v);
                            }
                            break;
                        case ItemsToChange.UiButtonType.StringButton:
                            {
                                var ifButtons = b.go.GetComponent<InputString>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, va); };//() => item.Override(true, "Canvas");

                                var v = item.GetValue();
                                ifButtons.ChangeValue((string)v);
                            }
                            break;
                        case ItemsToChange.UiButtonType.StringFunc:
                            {
                                var ifButtons = b.go.GetComponent<InputString>();
                                ifButtons.SetName(b.go.name);
                                ifButtons.onValueChange += (va) => { Execute(item, va); };//() => item.Override(true, "Canvas");
                            ...
                            break;
                        default:
                            throw new System.ArgumentOutOfRangeException("Type not supported " + b.bType);
                    }
                }
                catch(System.Exception e)
                {
                    
                    Debug.LogError("Problem with button for " + item + ": " + e);
                }
            }
        }
        ...
        public void ExecuteEnum(ItemsToChange item, object obj)
        {
            item.Input = obj;
            item.Override(true);

        }
        public void Execute(ItemsToChange item)
        {
            item.Override(true);
        }

        public void Execute(ItemsToChange item, float value)
        {
            item.Input = value;
            item.Override(true);
        }

        public void Execute(ItemsToChange item, int value)
        {
            item.Input = value;
            item.Override(true);
        }

        public void Execute(ItemsToChange item, bool value)
        {
            item.Input = value;
            item.Override(true);
        }

        public void Execute(ItemsToChange item, string value)
        {
            item.Input = value;
            item.Override(true);
        }
...
#if UNITY_EDITOR
#else
            if (runOnlyInEditor) Destroy(this);//Destroy this script
#endif
        }
