...
public class FastDebugWindow : EditorWindow
{
    ...
    private void ShowMember(ClassMemeberWithInfo mmbr, UnityEngine.Object script)
    {
        ...
        if (GUILayout.Button("Activate"))
        {
            if (useSelection)
            {
                GetRelevantObjects().ForEach(s => { try { mmbr.Activate(s); } catch (Exception e) { Debug.LogError("Probelm with debug call on " + s + ": " + e); } });
            }
            else
            {
        ...
        try
        {
            if (mmbr.MemberT == MemberTypes.Method)
                info = "()";
            else if (mmbr.MemberT == MemberTypes.Field)
                info =  " ["+ReflectionUtils.GetField(script, mmbr.Name).ToString() + "]";
            else if (mmbr.MemberT == MemberTypes.Property)
                info = " [" + ReflectionUtils.GetProperty(script, mmbr.Name).ToString() + "]";
        }
        catch (System.Exception ) { /*if (showLog) Debug.LogWarning(mmbr.Name + " Problem with showing data on " + e);*/ }


        var inputLbl = mmbr.Name + info + " ";// +  " " + " (" + mmbr.paramTypeDesc + ")";
    ...
    class Compre : IEqualityComparer<ClassMemeberWithInfo>
    {
        public bool Equals(ClassMemeberWithInfo a, ClassMemeberWithInfo b)
        {
            return a.Name.Equals(b.Name);
        }

        public int GetHashCode(ClassMemeberWithInfo obj)
        {
    ...
    private void UpdateMemebers()
    {
       ...
       // var set = new HashSet<ClassMemeberWithInfo>(memebers, new Compre());
        //memebers = set.ToArray();
        //System.Array.Sort(memebers, (a,b) => a.Name.CompareTo(b.Name));
        _selectedGo = GetRelevantObjects();
    }
    ...
    void OnGUI()
    {
        ...
        try
        {
            ...
            System.Array.Sort(sorted, (a,b) => a.Name.CompareTo(b.Name));
